...
def eq1(x,y,z,a):
    eq1=np.sin(y)-2*np.sin(x)-np.sin(x+y+z)+np.sin(z+y)-np.sin(x+y)-a
    return eq1
def eq2(x,y,z,b):
    eq2=np.sin(z)-2*np.sin(y)+np.sin(x)-np.sin(x+y)-np.sin(y+z)-b
    return eq2
def eq3(x,y,z,c):
    eq3=-2*np.sin(z)+np.sin(y)-np.sin(x+y+z)-np.sin(z+y)+np.sin(x+y)-c
    return eq3
for i in range(N):
    print(i)
    for j in range(N):
